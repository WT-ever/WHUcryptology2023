...
void SubBytes(unsigned char ciphertest[16])
{
    for (int i = 0; i < 16; i++) {
        ciphertest[i] = S[ciphertest[i]];
    }
};
...
void AddRoundKey (unsigned char ciphertext[16], unsigned char ki[16]) {
    for (int i = 0; i < 16; i++) {
        ciphertext[i] = ciphertext[i] ^ ki[i];
    }
}
...
void AES (unsigned char plaintext[16], unsigned char ciphertext[16], unsigned char k[11][16], int Rround) {
    ...
    for (int round = 1; round < Rround; round++) {
        ...
        SubBytes (ciphertext) ;
        if (round==1) {
            BytesToString(ciphertext, strs, 16);
            puts(strs);
            puts("ShiftRows");
        }
...
void InvSubBytes(unsigned char plaintext[16]) {
    for (int i = 0; i < 16; i++) {
        plaintext[i] = IS[plaintext[i]];
    }
}
...
void InvShiftRows (unsigned char* a) {
    unsigned char b[16];
    b[0] = a[0];  b[4] = a[4];  b[8] = a[8];  b[12] = a[12];
    b[5] = a[1];  b[9] = a[5];  b[13] = a[9]; b[1] = a[13];
    b[10] = a[2]; b[14] = a[6]; b[2] = a[10]; b[6] = a[14];
    b[15] = a[3]; b[3] = a[7];  b[7] = a[11]; b[11] = a[15];

    for (int i = 0; i < 16; i++) 
        a[i] = b[i];
}

void InvKeyExpansion(unsigned char K[16], unsigned char invk[11][16]) {
    keyExpansion(K, invk);
    for (int i = 1; i<10; i++) 
        InvMixColumns(invk[i]);
}
void InvAES(unsigned char ciphertext[16], unsigned char plaintext[16], unsigned char k[11][16], int Round)
{
    for (int i = 0; i < 16; i++)
    {
