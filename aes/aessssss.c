#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <time.h>
#include <math.h>
#define DEBUGK
#define DEBUGKK
#define N 100000

// S盒
unsigned char S[256] =
    {
        };

unsigned char S2[256] =
    {};

unsigned char S3[256] =
    {};

// 轮常数
int RC[10] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

void SubBytes(unsigned char ciphertext[16])
{
    for (int i = 0; i < 16; i++)
    {
        ciphertext[i] = S[ciphertext[i]];
    }
}

void ShiftRows(unsigned char *a)
{
    unsigned char b[16];
    b[0] = a[0];
    b[4] = a[4];
    b[8] = a[8];
    b[12] = a[12];
    b[1] = a[5];
    b[5] = a[9];
    b[9] = a[13];
    b[13] = a[1];
    b[2] = a[10];
    b[6] = a[14];
    b[10] = a[2];
    b[14] = a[6];
    b[3] = a[15];
    b[7] = a[3];
    b[11] = a[7];
    b[15] = a[11];
    for (int i = 0; i < 16; i++)
    {
        a[i] = b[i];
    }
}

void MixColumns(unsigned char *a)
{
    unsigned char b[16];
    b[0] = S2[a[0]] ^ S3[a[1]] ^ a[2] ^ a[3];
    b[1] = S2[a[1]] ^ S3[a[2]] ^ a[3] ^ a[0];
    b[2] = S2[a[2]] ^ S3[a[3]] ^ a[0] ^ a[1];
    b[3] = S2[a[3]] ^ S3[a[0]] ^ a[1] ^ a[2];
    b[4] = S2[a[4]] ^ S3[a[5]] ^ a[6] ^ a[7];
    b[5] = S2[a[5]] ^ S3[a[6]] ^ a[7] ^ a[4];
    b[6] = S2[a[6]] ^ S3[a[7]] ^ a[4] ^ a[5];
    b[7] = S2[a[7]] ^ S3[a[4]] ^ a[5] ^ a[6];
    b[8] = S2[a[8]] ^ S3[a[9]] ^ a[10] ^ a[11];
    b[9] = S2[a[9]] ^ S3[a[10]] ^ a[11] ^ a[8];
    b[10] = S2[a[10]] ^ S3[a[11]] ^ a[8] ^ a[9];
    b[11] = S2[a[11]] ^ S3[a[8]] ^ a[9] ^ a[10];
    b[12] = S2[a[12]] ^ S3[a[13]] ^ a[14] ^ a[15];
    b[13] = S2[a[13]] ^ S3[a[14]] ^ a[15] ^ a[12];
    b[14] = S2[a[14]] ^ S3[a[15]] ^ a[12] ^ a[13];
    b[15] = S2[a[15]] ^ S3[a[12]] ^ a[13] ^ a[14];
    for (int i = 0; i < 16; i++)
    {
        a[i] = b[i];
    }
}

void AddRoundKey(unsigned char ciphertext[16], unsigned char k[16])
{
    for (int i = 0; i < 16; i++)
    {
        ciphertext[i] ^= k[i];
    }
}

void Generate_Keys(unsigned char k[11][16], unsigned char K[16])
{
    for (int i = 0; i < 16; i++)
    {
        k[0][i] = K[i];
    }
    for (int i = 1; i < 11; i++)
    {
        k[i][0] = k[i - 1][0] ^ S[k[i - 1][13]] ^ RC[i - 1];
        k[i][1] = k[i - 1][1] ^ S[k[i - 1][14]];
        k[i][2] = k[i - 1][2] ^ S[k[i - 1][15]];
        k[i][3] = k[i - 1][3] ^ S[k[i - 1][12]];
        k[i][4] = k[i - 1][4] ^ k[i][0];
        k[i][5] = k[i - 1][5] ^ k[i][1];
        k[i][6] = k[i - 1][6] ^ k[i][2];
        k[i][7] = k[i - 1][7] ^ k[i][3];
        k[i][8] = k[i - 1][8] ^ k[i][4];
        k[i][9] = k[i - 1][9] ^ k[i][5];
        k[i][10] = k[i - 1][10] ^ k[i][6];
        k[i][11] = k[i - 1][11] ^ k[i][7];
        k[i][12] = k[i - 1][12] ^ k[i][8];
        k[i][13] = k[i - 1][13] ^ k[i][9];
        k[i][14] = k[i - 1][14] ^ k[i][10];
        k[i][15] = k[i - 1][15] ^ k[i][11];
    }
}

void AES(unsigned char plaintext[16], unsigned char ciphertext[16], unsigned char k[11][16], int Round)
{
    for (int i = 0; i < 16; i++)
    {
        ciphertext[i] = plaintext[i];
    }
    AddRoundKey(ciphertext, k[0]);
    for (int round = 1; round < Round; round++)
    {
        SubBytes(ciphertext);
        ShiftRows(ciphertext);
        MixColumns(ciphertext);
        AddRoundKey(ciphertext, k[round]);
    }
    SubBytes(ciphertext);
    ShiftRows(ciphertext);
    AddRoundKey(ciphertext, k[Round]);
}

void H_StringToHex(char str[32], unsigned char strhex[32])
{
    char c = str[0];
    for (int i = 0; i < 32; i++)
    {
        c = str[i];
        if ((c >= '0') && (c <= '9'))
        {
            strhex[i] = c - '0';
        }
        if ((c >= 'A') && (c <= 'F'))
        {
            strhex[i] = c - 'A' + 10;
        }
        if ((c >= 'a') && (c <= 'f'))
        {
            strhex[i] = c - 'a' + 10;
        };
    }
}

void B_StringToHex(char str[128], unsigned char strhex[32])
{
    for (int i = 0; i < 32; i++)
    {
        strhex[i] = ((str[i * 4] - '0') << 3) + ((str[i * 4 + 1] - '0') << 2) + ((str[i * 4 + 2] - '0') << 1) + (str[i * 4 + 3] - '0');
    }
}

int main(void)
{
    unsigned char plaintext[16];
    unsigned char ciphertext[16];
     unsigned char K[16];
     unsigned char k[11][16];
     int Round = 10;

     // 按十六进制连续输入
     char plain[32];
     printf("输入明文：0x");
     gets(plain);
     unsigned char hex_plain[32];
     H_StringToHex(plain, hex_plain);
     for (int i = 0; i < 16; i++)
     {
         plaintext[i] = (hex_plain[i * 2] << 4) + hex_plain[i * 2 + 1];
     }

     char keys[32];
     printf("输入密钥：0x");
     gets(keys);
     unsigned char hex_keys[32];
     H_StringToHex(keys, hex_keys);
     for (int i = 0; i < 16; i++)
     {
         K[i] = (hex_keys[i * 2] << 4) + hex_keys[i * 2 + 1];
     }

     #ifdef DEBUGKK
     for (int i = 0; i < 16; i++) {
        printf("%d ", K[i]);
     }
     putchar('\n');
     #endif

    // // // 按二进制连续输入
    // // char plain[128];
    // // printf("输入明文：0b");
    // // gets(plain);
    // // unsigned char hex_plain[32];
    // // B_StringToHex(plain, hex_plain);
    // // for (int i = 0; i < 16; i++)
    // // {
    // //     plaintext[i] = (hex_plain[i * 2] << 4) + hex_plain[i * 2 + 1];
    // // }

    // // char keys[128];
    // // printf("输入密钥：0b");
    // // gets(keys);
    // // ElemType hex_keys[32];
    // // B_StringToHex(keys, hex_keys);
    // // for (int i = 0; i < 16; i++)
    // // {
    // //     K[i] = (hex_keys[i * 2] << 4) + hex_keys[i * 2 + 1];
    // // }

     Generate_Keys(k, K);
     #ifdef DEBUGK
     for (int i = 0; i < 11; i++) {
        printf("%d: ", i);
        for (int j = 0; j < 16; j++) {
            printf("%d ", k[i][j]);
        }
        putchar('\n');
     }
     #endif
     AES(plaintext, ciphertext, k, Round);

     // 按十六进制输出
     printf("密文为：0x");
     for (int i = 0; i < 16; i++)
     {
         printf("%02x", ciphertext[i]);
     }
     printf("\n");

    // // // 按二进制输出
    // // printf("密文为：0b");
    // // for (int i = 0; i < 16; i++)
    // // {
    // //     unsigned char tmp = ciphertext[i];
    // //     for (int j = 0; j < 8; j++)
    // //     {
    // //         int c = tmp / 128;
    // //         printf("%d", c);
    // //         tmp = tmp << 1;
    // //     }
    // // }
    // // printf("\n");

    // test_1
    /*
    unsigned char plaintext1[16];
    unsigned char ciphertext1[16];
    unsigned char plaintext2[16];
    unsigned char ciphertext2[16];
    unsigned char compare[16];
    unsigned char K[16];
    unsigned char k[11][16];
    int Round = 10;
    int cnt = 0;
    int sum = 0;
    srand((unsigned)time(NULL));
    for (int i = 0; i < 16; i++)
    {
        K[i] = (unsigned char)(i % 8 + 1);
    }
    Generate_Keys(k, K);

    for (int loop = 0; loop < N; loop++)
    {
        for (int i = 0; i < 16; i++)
        {
            unsigned char num = rand() % 255;
            plaintext1[i] = num;
        }
        AES(plaintext1, ciphertext1, k, Round);
        for (int i = 0; i < 128; i++)
        {
            cnt = 0;
            for (int j = 0; j < 16; j++)
            {
                plaintext2[j] = plaintext1[j];
            }
            plaintext2[i / 16] = plaintext2[i / 16] ^ (unsigned char)pow(2, (i % 8));
            AES(plaintext2, ciphertext2, k, Round);
            for (int j = 0; j < 16; j++)
            {
                compare[j] = ciphertext1[j] ^ ciphertext2[j];
                for (int k = 0; k < 8; k++)
                {
                    if (compare[j] % 2 != 0)
                    {
                        cnt++;
                    }
                    compare[j] >>= 2;
                }
            }
            sum += cnt;
        }
    }
    printf("明文输入改变1位，密文输出平均改变的位数为：%f\n", sum / (128.0 * N));
    */

    return 0;
}
